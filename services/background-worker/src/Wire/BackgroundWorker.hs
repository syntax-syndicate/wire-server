...
supervisor :: forall a. Env -> String -> AppT IO a -> IO (Async (Maybe a))
...
run opts = do
  ...
  (notifChanRef, notifConsumersRef) <- runAppT env $ BackendNotificationPusher.startWorker amqpEP -- TODO: now do this one!
